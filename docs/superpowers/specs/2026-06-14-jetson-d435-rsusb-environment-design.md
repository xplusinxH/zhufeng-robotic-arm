...
外部存储挂载于 `/media/jetson/1896-8302`，文件系统为 `vfat`。该文件系统不支持
Linux 符号链接，因此只能保存普通项目文件，不能用于 Git 仓库或 Linux 动态库构建。
目录分配如下：
...
- 测试图像与实验数据：`/media/jetson/1896-8302/zhufeng/data`，预算 12 GB。
...
需要符号链接的第三方源码和构建目录临时放在 eMMC 的 `/home/jetson/build`。
构建完成并安装至系统路径后，删除对应源码和构建目录，恢复 eMMC 空间。
...
- 禁止在 `vfat` 外部存储中执行 Git 克隆或 Linux 动态库构建。
- 所有编译产物保存在外部存储中。
